@props(['items' => []])
<nav class="flex items-center gap-1.5 text-sm text-slate-500 mb-4" aria-label="Breadcrumb">
    ...
    @foreach($items as $item)
        ...
        @if($item['url'] ?? false)
            <a href="{{ $item['url'] }}" class="hover:text-indigo-600 transition-colors">{{ $item['label'] }}</a>
        @else
