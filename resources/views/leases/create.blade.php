    ...
    <div class="p-6">
        <form action="{{ route('leases.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="md:col-span-2">
                    <x-forms.select name="tenant_id" label="Tenant" required :options="$tenants->mapWithKeys(fn($t) => [$t->id => $t->user->name])->toArray()" placeholder="Select Tenant" />
                </div>
                ...
                <div class="md:col-span-4">
                    <x-forms.button variant="primary">Create Lease</x-forms.button>
                </div>
            </div>
        </form>
    </div>
