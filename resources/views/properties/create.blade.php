...
<x-breadcrumbs :items="[['label' => __('Properties'), 'url' => route('properties.index')], ['label' => __('Add Property')]]" />
        ...
        <form action="{{ route('properties.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                <div class="md:col-span-6">
                    <x-forms.input name="name" label="{{ __('Property Name') }}" :value="old('name')" required />
                </div>
                <div class="md:col-span-3">
                    <x-forms.select name="type" label="{{ __('Type') }}" required :options="\App\Models\Property::propertyTypes()" placeholder="Select type..." x-data @change="if($el.value) { const city = document.querySelector('[name=city]'); if(city && city.value) { const evt = new Event('change', {bubbles: true}); city.dispatchEvent(evt); } }" />
                </div>
                <div class="md:col-span-3">
                    <x-forms.select name="status" label="{{ __('Status') }}" :options="['active' => __('Active'), 'inactive' => __('Inactive'), 'under_maintenance' => __('Under Maintenance')]" :value="old('status', 'active')" />
                </div>

                <div class="md:col-span-12">
                    <x-forms.textarea name="description" label="{{ __('Description') }}" :value="old('description')" rows="4" />
                </div>
                ...
                <div class="md:col-span-6">
                    <x-forms.input name="address" label="{{ __('Address') }}" :value="old('address')" required />
                </div>
                <div class="md:col-span-3">
                    <x-forms.select name="city" label="{{ __('City') }}" required :options="\App\Models\Property::cities()" placeholder="Select city..." x-data @change="const d = document.querySelector('[name=district]'); if(d) { fetch('/api/locations/districts?city='+encodeURIComponent($el.value)).then(r=>r.json()).then(data=>{ d.innerHTML='<option value=\"\">Select district...</option>'+data.map(x=>'<option value=\"'+x+'\">'+x+'</option>').join(''); }).catch(()=>{}); }" />
                </div>
                <div class="md:col-span-3">
                    <x-forms.select name="district" label="{{ __('District') }}" :options="[]" placeholder="Select district..." />
                </div>
                ...
                <div class="md:col-span-4">
                    <x-forms.input name="country" label="{{ __('Country') }}" :value="old('country', 'Cameroon')" />
                </div>
                <div class="md:col-span-4">
                ...
                <div class="md:col-span-4">
                    <x-forms.input name="total_units" label="{{ __('Total Units') }}" type="number" :value="old('total_units', 1)" min="1" />
                </div>
                <div class="md:col-span-4">
                ...
                <div class="md:col-span-6">
                    <x-forms.group label="{{ __('Property Images') }}">
                        <div class="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-6 text-center hover:border-indigo-400 transition-colors" x-data="{ dragging: false }" @@dragover.prevent="dragging = true" @@dragleave.prevent="dragging = false" @@drop.prevent="dragging = false">
                            ...
                            <button type="button" @@click="$refs.imageInput.click()" class="mt-3 btn-secondary btn-sm">{{ __('Browse Files') }}</button>
                        </div>
                ...
                <div class="md:col-span-6 flex items-end">
                    <x-forms.checkbox name="featured" :checked="old('featured')">{{ __('Mark as Featured Property') }}</x-forms.checkbox>
                </div>
                ...
                <div class="md:col-span-12 border-t border-slate-200 dark:border-slate-700 pt-4 flex items-center gap-3">
                    ...
                    <a href="{{ route('properties.index') }}" class="btn-secondary">{{ __('Cancel') }}</a>
                </div>
            </div>
        </form>
    </div>
