        ...
        <form action="{{ route('tenants.update', $tenant) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                <div class="md:col-span-6">
                    <x-forms.select name="user_id" label="User" :options="\App\Models\User::where('role','tenant')->get()->mapWithKeys(fn($u) => [$u->id => $u->name])->toArray()" :value="old('user_id', $tenant->user_id)" />
                </div>
                <div class="md:col-span-6">
                    <x-forms.select name="unit_id" label="Unit" :options="$units->mapWithKeys(fn($u) => [$u->id => $u->unit_number.' - '.$u->property->name])->toArray()" :value="old('unit_id', $tenant->unit_id)" />
                </div>
                ...
                <div class="md:col-span-4">
                    <x-forms.select name="status" label="Status" :options="['pending' => 'Pending', 'active' => 'Active', 'past' => 'Past']" :value="old('status', $tenant->status)" />
                </div>
