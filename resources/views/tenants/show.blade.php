...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 mb-4">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="text-sm text-slate-600"><strong>{{ __('Name') }}:</strong> {{ $tenant->user->name }}</div>
                    <div class="text-sm text-slate-600"><strong>{{ __('Email') }}:</strong> {{ $tenant->user->email }}</div>
                    <div class="text-sm text-slate-600"><strong>{{ __('Phone') }}:</strong> {{ $tenant->user->phone ?? 'N/A' }}</div>
                    <div class="text-sm text-slate-600"><strong>{{ __('Status') }}:</strong> <span class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium rounded-full {{ $tenant->status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">{{ ucfirst($tenant->status) }}</span></div>
                    <div class="text-sm text-slate-600"><strong>{{ __('Unit') }}:</strong> {{ $tenant->unit->unit_number }} ({{ $tenant->unit->property->name }})</div>
                    <div class="text-sm text-slate-600"><strong>{{ __('Rent') }}:</strong> ${{ number_format($tenant->unit->rent_amount, 2) }}</div>
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 mb-4">
            <div class="px-6 py-4 border-b border-slate-200"><h5 class="text-lg font-semibold text-slate-800">{{ __('Recent Payments') }}</h5></div>
            <div class="p-6">
