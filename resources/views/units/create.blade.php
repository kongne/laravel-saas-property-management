        ...
        <form action="{{ route('units.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                <div class="md:col-span-6">
                    <x-forms.select name="property_id" label="Property" required :options="$properties->mapWithKeys(fn($p) => [$p->id => $p->name])->toArray()" placeholder="Select Property" />
                </div>
                <div class="md:col-span-3"><x-forms.input name="unit_number" label="Unit Number" required /></div>
                <div class="md:col-span-3">
                    <x-forms.select name="type" label="Type" :options="['studio' => 'Studio', 'one_bedroom' => '1 Bedroom', 'two_bedroom' => '2 Bedroom', 'three_bedroom' => '3 Bedroom', 'penthouse' => 'Penthouse', 'commercial' => 'Commercial', 'other' => 'Other']" :value="old('type', 'studio')" />
                </div>
                ...
                <div class="md:col-span-12"><x-forms.textarea name="description" label="Description" rows="2" /></div>
